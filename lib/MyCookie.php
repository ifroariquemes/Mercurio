...
class MyCookie
{
    ...
    {
        $mConf = $this->getModuleConfiguration($this->module);
        if (isset($this->URLVariables[1]) &&
                array_key_exists($this->URLVariables[1], $mConf->getSubmodules())) {
    ...
    private function setActionCustom()
    {
        ...
        if (!empty($this->submodule) && $this->getURLVariablesLength() === 2) {
            $this->action = $moduleConfig->getController($this->submodule)->getHome();
        } else if ($this->getURLVariablesLength() === 1 ||
    ...
    {
        if (!class_exists($class) && !empty($this->submodule)) {
            $class = str_replace($this->submodule . '\\', '', $class);
        }
        if (!class_exists($class)) {
            $class = $this->getControlClass($this->URLVariables[0]);
            if (empty($class)) {
                throw new \Exception("There is no default control class set to $this->gateway");
            }
        }
        $methods = get_class_methods($class);        
        $methodIndex = (empty($this->submodule)) ? 1 : 2;
        if ($this->getURLVariablesLength() > $methodIndex) {                        
            $possibleAction = str_replace('_', '', $this->URLVariables[$methodIndex]);
            if (in_array($possibleAction, $methods)) {
                $this->action = $action = $possibleAction;
            }
        }
        if(empty($action)) {
            if(empty($this->submodule)) {
                throw new \Exception("There is no default action set to '$this->module' module");
            } else {
                throw new \Exception("There is no default action set to '$this->submodule' submodule");
            }           
        }
        ob_start();
        $object = new $class;
